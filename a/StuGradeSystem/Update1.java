...
import javax.swing.*;
import java.sql.*;
public class Update1 extends JPanel {
		...
		button1.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				if(number.getText().toString().equals(""))
					JOptionPane.showMessageDialog(null, "账号不能为空","修改",JOptionPane.WARNING_MESSAGE);
				else
				{
					try{
						...
						sqlstr="select *from Admin where num='"+strnum+"'";
					//创建数据库连接
					...
					if(result.next())
					{
						number.setText(result.getString("num"));
						...
						JOptionPane.showMessageDialog(null, "没有该账号的用户","修改",JOptionPane.WARNING_MESSAGE);
							...
							JOptionPane.showMessageDialog(null, "密码不能为空","修改",JOptionPane.WARNING_MESSAGE);	
									...
									else
										{
											try
											{
												...
												JdbcFile conn=new JdbcFile();
												//产生登录SQL语句
												strsql="update Admin set password1='"+strpassword+"'where num='"+strnum+"'";
